...
export default function Audit() {
  const [events, setEvents] = useState<Audit[] | null>(null);

  useEffect(() => {
    api<Audit[]>("/api/v1/audit").then(setEvents).catch(() => setEvents([]));
  }, []);
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        {events.length === 0 ? (
          <Empty title="No audit events" body="Authorization decisions will appear here." />
        ) : (
